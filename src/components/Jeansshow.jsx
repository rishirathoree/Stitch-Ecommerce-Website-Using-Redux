...
const Jeansshow = () => {
    const clothingItems = useSelector((state) => state.data.filter((item) => item.category === 'Jeans'));
  return (
    <>
    <div className='p-8 w-full h-full'>
        <p className='font-bold text-[15px]'>Jeans</p>
        <div className='w-full h-full grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-4'>
            {clothingItems.map((items,index)=>{
                return(
                    <Link key={items.id} to={`/category/jeans`}>
